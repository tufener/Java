...
public class Db {
	...
	static String selectByNom(String nomSaisie) throws ClassNotFoundException, SQLException{
		String res = "ID\tNOM\tPRENOM\n";
		Class.forName("com.mysql.jdbc.Driver");
		Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/cgi", "root", "root");
	...
	static void testDelete(int id) throws ClassNotFoundException, SQLException {
		...
		Statement st = conn.createStatement();
		int compt = st.executeUpdate(sql); // on crée une condition car lorsque
		// nous refaisons delete on a pas
		// d'erreur comparé aux autres
		// méthodes où ils nous renvoyaient
		// une erreur
		if (compt != 0)
