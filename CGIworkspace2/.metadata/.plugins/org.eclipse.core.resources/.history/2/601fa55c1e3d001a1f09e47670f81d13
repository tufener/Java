...
public abstract class Utils {
	...
	{
		if(a<0||b<0)throw new NegException("Valeur négative détectée",a,b);
		switch(op) {
		case "":throw new OpException("Opérateur non renseigné", op);
		case "+":
		case "add":
		...
		default :
			throw new OpException("Opérateur inconnu",op);
		}
	}
	...
	private static double calcDiv(double a, double b) throws ArithmeticException{
		if(b==0)throw new ArithmeticException("On a dit, PAS DE DIVISION PAR 0!!!!!");
		return a/b;
	}
