...
public class CompteSimple {
	...
	public boolean crediter(int s) throws OperationNegException {
		...
		if (s < 0) {
			throw new OperationNegException("operation négative", "méthode créditer " + s);
		}
		solde += (s - COMMISSION);
		success = true;
		return success;
	...
	public boolean debiter(int s) throws OperationNegException, SoldeInsuffisantException {
		boolean success = false;
		if (s < 0)
			throw new OperationNegException("operation négative", "méthode débiter " + s);
		else if ((s + COMMISSION) >= this.solde) {
			throw new SoldeInsuffisantException("solde insuffisant", "méthode débiter"+s);
		}
		solde -= (s + COMMISSION);
		success = true;
		return success;
	...
	public boolean virement(int somme, CompteSimple c) throws SoldeInsuffisantException, OperationNegException {
		...
		try {
			...
			throw new SoldeInsuffisantException(e.getMessage(), "méthode virement"+somme);
		} catch (OperationNegException e) {
			throw new OperationNegException(e.getMessage(), "méthode virement"+somme);
		}
		return success;
	...
	public String toString() {
		return "________________\n" + this.getClass().getSimpleName() + " du client: " + nom + "\nsolde: " + solde
				+ "€";
	}
