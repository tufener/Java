...
public class TpTableau {
	...
	static void tpTableauJacky() {
		...
		for (int i = 0; i < Tab.length; i++) // on peut aussi écrire ==>
												// for(String e:Tab)<== --> à la
	...
	static void afficheTableau(int[] x) { // on a donné dans les paramètres les
	...
	static void testDoubleTableauv3() { // autre méthode pour changer le tableau
										// d'origine...
		int[] tab1 = { 10, 20, 30, 40 };
		afficheTableau(tab1);// 10 20 30 40
	...
	static int[] doubleTableauv3(int[] x) { // autre méthode pour changer le
											// tableau d'origine...
		for (int i = 0; i < x.length; i++)
			x[i] *= 2;
		return x;
